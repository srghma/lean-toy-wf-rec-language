...
* **calls to other functions**: non-recursive ones are inlined (`double` in `sumDoubles`),
  recursive ones become nested local recursive functions (`gcd` in `gcdSum`, `lcmGcd`,
  `countCoprime`; `fact` in `sumFacts`; several of them in `chain`; two loops in `twoLoops`;
  `Tco.mc91Loop` in the uploaded `Tco.mc91TR`);
* a few functions that must be **rejected** (`Mutual.isEven`/`isOdd`: mutual recursion;
  `gcdLoop`: a call inside a `Tail` loop body).
...
/-- A call of `gcd` in the body of a tail-recursive function: fine for `PCL` and `Meas`, but not
for `Tail` (a loop body cannot contain another loop). -/
