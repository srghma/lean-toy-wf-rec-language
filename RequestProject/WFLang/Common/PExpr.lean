...
operators, `!` and `if-then-else`.  They are the leaves of the `PCL` and `Tail` grammars and the
measures of `Meas`.  `PExprs.ids Γ` is the argument tuple made of all the variables of `Γ`.
-/
