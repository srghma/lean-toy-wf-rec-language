...
/-!
# `#lean_wf_func_to_pcl f` — capture a well-founded Lean function as a `PCL.Term`

```
def gcd_term : PCL.Term ⟨[.nat, .nat], .nat⟩ := #lean_wf_func_to_pcl gcd
theorem gcd_agree : ∀ m n, PCL.Term.eval gcd_term m n = gcd m n := by pcl_agree
```
...
partial def lift (c : Ctx) (e : Lean.Expr) (k : Ctx → Lean.Expr → TermElabM Stx) :
  ...
  throwError "#lean_wf_func_to_pcl: recursive call in an unsupported position{indentExpr e}"
...
partial def fixHead (fn : Name) (visiting : List Name := []) : TermElabM Stx := do
  if visiting.contains fn then
    throwError "#lean_wf_func_to_pcl: mutual recursion through {fn} is not supported"
  let (argTys, retTy) ← signatureOf fn
  let gam := mkTyList argTys
  let some (R, wf, lemmas) ← closedFixOf fn |
    throwError "#lean_wf_func_to_pcl: {fn} is not defined by well-founded recursion"
...
/-- `#lean_wf_func_to_pcl f` -/
syntax (name := toPcl) "#lean_wf_func_to_pcl " ident : term

@[term_elab toPcl] def elabToPcl : TermElab := fun stx expectedType? => do
  let fn ← realizeGlobalConstNoOverloadWithInfo stx[1]
  elabTerm (← captureStx fn) expectedType?

/-- `pcl_agree` proves `∀ x₁ … xₙ, PCL.Term.eval f_term x₁ … xₙ = f x₁ … xₙ` for a term
produced by `#lean_wf_func_to_pcl f`: by uniqueness of the solution of the `fix` equation
(`Term.eval_fix_eq`) it suffices that `f` satisfies the equation of the body, which follows
from `f.eq_def`. -/
syntax (name := pclAgree) "pcl_agree" : tactic
...
@[tactic pclAgree] def evalPclAgree : Tactic.Tactic := fun _ => do
  let (t, f, eqDef) ← agreeTarget "pcl_agree"
